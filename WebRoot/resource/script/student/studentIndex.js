$(function(){
	...
	$("#delete-btn").click(function(){
		var idList=[];
		idList=getDeleteIdList();
		if(idList.length == 0){
			$("#mymodal .modal-body p").html("未选中任何课程！");	
			$("#mymodal").modal();
			return;
		}
		$("#mymodal2 .modal-body p").html("确认要删除" + idList.length + "门课程吗？");	
		$("#mymodal2").modal();					
	});	
...
//ajax 请求，删除课程--数据库
function ajaxDeleteCourseList(idList) {
	$.ajax({
		type : "post",
		...
		success : function(result) {
			if(result["resultFlag"] == "DS"){	
				$("#mymodal .modal-body p").html("删除课程成功！");	
				$("#mymodal").modal();
			}
			else {
				$("#mymodal .modal-body p").html("删除课程失败！");	
				$("#mymodal").modal();
			}
		}
	});
